...
class VariablesModel
{
    ...
    {
        $sql = "SELECT id, CAST(value AS CHAR) AS value
    ...
    /* ======== HISTÓRICO (listados completos opcionales) ======== */
    public function listDebitCreditHistory(int $limit = 50)
    {
        $stmt = $this->conn->prepare("SELECT id, CAST(value AS CHAR) AS value
                                      FROM env_debit_credit_tax
                                      ORDER BY id DESC
                                      LIMIT :lim");
    ...
    {
        $stmt = $this->conn->prepare("SELECT id, name, cuit
                                      FROM env_billing_entities
                                      ORDER BY id DESC
                                      LIMIT :lim");
        $stmt->bindValue(':lim', $limit, PDO::PARAM_INT);
    ...
    {
        // Nota: hay UNIQUE en CUIT. Si querés histórico con el mismo CUIT, habrá que ajustar el schema.
        try {
