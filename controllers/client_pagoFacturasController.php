...
require_once __DIR__ . '/../core/SessionManager.php'; // <— OJO: un solo ".."
 ...
 * - Para JSON: carga en $input
 * - Para multipart: usa $_POST y $_FILES
 */
$raw    = file_get_contents('php://input');
$ctype  = $_SERVER['CONTENT_TYPE'] ?? '';
$isJson = stripos($ctype, 'application/json') !== false;

// Si es JSON, $input viene de json_decode; si no, de $_POST
$input  = $isJson ? (json_decode($raw, true) ?? []) : $_POST;
$action = $input['action'] ?? '';

/** Helper: sanitizar a número (soporta coma como decimal) */
$toFloat = function ($v) {
...
try {
    ...
    switch ($action) {
        ...
        case 'create': {
                // Debe venir como multipart/form-data (por los PDFs)
                $data = [
